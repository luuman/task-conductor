...
from .engine import get_task_stages, STAGE_ORDER, APPROVAL_REQUIRED as _ENGINE_APPROVAL
...
async def run_pipeline(task_id: int, worktree_path: str):
    ...
    with Session(engine) as db:
        ...
        project_stages_config = project.stages_config if project else None
        task_stages = get_task_stages(task, project_stages_config)
