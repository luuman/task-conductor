...
impl BrowserTransport {
    ...
    ///
    /// 注意：重连逻辑由调用方（ws-worker.js）负责，Rust 层只负责单次连接管理
    pub fn new(
        url: &str,
...
- [ ] **Step 4: 先将 WASM 构建产物加入 .gitignore，防止意外提交**

```bash
cd /home/sichengli/Documents/code2/task-conductor
grep -q "ws-core/pkg/" .gitignore || echo "ws-core/pkg/" >> .gitignore
git add .gitignore
git commit -m "chore: exclude ws-core WASM build artifacts from git"
```

- [ ] **Step 5: 验证 WASM 编译（需要 wasm-pack）**
...
预期：`ws-core/pkg/` 目录生成 `ws_core_bg.wasm`、`ws_core.js`、`ws_core.d.ts`
...
```bash
cd /home/sichengli/Documents/code2/task-conductor
git add ws-core/src/transport/browser.rs ws-core/src/wasm.rs ws-core/src/lib.rs ws-core/src/transport/mod.rs
...
- [ ] **Step 3: 在 .gitignore 中补充 target/ 排除（pkg/ 已在 Task 6 中处理）**

```bash
cd /home/sichengli/Documents/code2/task-conductor
grep -q "ws-core/target/" .gitignore || echo "ws-core/target/" >> .gitignore
git add .gitignore
git commit -m "chore(ws-core): exclude Rust target dir from git"
```
