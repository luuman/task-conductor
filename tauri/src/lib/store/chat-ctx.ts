...
export function createLocalChatStore(): StoreApi<ChatStore> {
  return createStore<ChatStore>()((set) => ({
    ...
    setProjectCwd:    (cwd: string | null) => set({ projectCwd: cwd }),
    setActiveTaskId:  (id: number | null) => set({ activeTaskId: id, messages: [], currentReply: '' }),
    addMessage:       (msg) => set((s) => ({ messages: [...s.messages, msg] })),
    setMessages:      (msgs) => set({ messages: msgs }),
    setCurrentReply:  (text: string) => set({ currentReply: text }),
    appendCurrentReply: (text: string) => set((s) => ({ currentReply: s.currentReply + text })),
    setIsGenerating:  (v: boolean) => set({ isGenerating: v }),
    setSystemPrompt:  (prompt: string) => set({ systemPrompt: prompt }),
    setClaudeSessionId: (id: string | null) => set({ claudeSessionId: id }),
    setPrd:           (prd: string | null) => set({ prdContent: prd }),
