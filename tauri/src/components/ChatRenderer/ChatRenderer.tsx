...
import { cleanSystemXml } from '../../features/chat/timeline-parser'
...
export function CopyButton({ text, getText, className }: { text?: string; getText?: () => string; className?: string }) {
...
export function CodeBlock({ code = '', lang, label, icon, action, fileName, variant, pillColor, children }: CodeBlockProps) {
  ...
  const header = (() => {
    switch (variant) {
      ...
      case 8: return (
        <div className={`${styles.codeHeader} ${variantCls}`}>
          <span className={styles.cbTag} style={pillColor ? { borderColor: pillColor, color: pillColor } : undefined}>{action}</span>
          {icon}<span className={styles.cbFile}>{fileName}</span>
          <span style={{ flex: 1 }} />
          {canCollapse && <span className={styles.codeLines}>{linesText}</span>}
          {copyBtn}{toggleBtn}
