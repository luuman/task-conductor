                ...
                <div className="bg-app rounded-lg p-2.5 space-y-1.5">
                  ...
                  <div className="flex items-center gap-2">
                    <code className="flex-1 text-[11px] font-mono text-green-400 break-all">
                      {`ssh -L 8765:localhost:8765 ${sshInfo.user}@${sshInfo.host} -p ${sshInfo.port}`}
                    </code>
                    <button
                      onClick={() => {
                        navigator.clipboard.writeText(`ssh -L 8765:localhost:8765 ${sshInfo.user}@${sshInfo.host} -p ${sshInfo.port}`);
                        setCopied(true);
