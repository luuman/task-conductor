    ...
    <div>
      ...
      <div className="border border-dashed border-app rounded-xl px-4 py-3 space-y-2 mt-3">
        ...
        <div className="flex gap-2">
          ...
          <input value={newValue} onChange={e => setNewValue(e.target.value)} onKeyDown={e => e.key === "Enter" && handleAdd()} placeholder={t("claudeConfig.otherConfig.value")} spellCheck={false}
...
function SecSettings() {
  ...
  return (
    <div className="space-y-4">
      <SectionHeader icon={Cpu} color="#8b5cf6" label={t("claudeConfig.navItems.modelAndParams")} desc={t("claudeConfig.sectionDesc.settings")} />
      <GlobalConfigPanel />
    </div>
  );
}
...
function SecEnvVars() {
  ...
  const DEFS = [
    ...
    { key: "ANTHROPIC_MODEL", desc: "Model" },
    { key: "HTTPS_PROXY", desc: "HTTPS Proxy" },
    { key: "HTTP_PROXY", desc: "HTTP Proxy" },
    { key: "NO_PROXY", desc: "No Proxy" },
    { key: "CLAUDE_CONFIG_DIR", desc: "Config Dir" },
    { key: "CLAUDE_CACHE_DIR", desc: "Cache Dir" },
    { key: "CLAUDE_LOG_LEVEL", desc: "Log Level" },
    { key: "CLAUDE_NO_COLOR", desc: "No Color" },
    { key: "CLAUDE_EDITOR", desc: "Editor" },
    { key: "DEBUG", desc: "Debug" },
  ];
  return (
    <div className="space-y-4">
      <SectionHeader icon={Variable} color="#a855f7" label={t("claudeConfig.envVars.title")} desc={t("claudeConfig.sectionDesc.envVars")} />
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-3">
        {DEFS.map(env => (
...
function SecAbout({ systemInfo, overview }: { systemInfo: ClaudeSystemInfo | null; overview: ClaudeOverview | null }) {
  ...
  const checkUpdate = async () => {
    ...
    try {
      ...
      else { setUpdateInfo(`Latest: ${latest}, run: npm update -g @anthropic-ai/claude-code`); }
    } catch { setUpdateInfo("Check failed, run: claude --version"); }
...
function SecTrash({ items, onRefresh }: { items: DisabledItem[]; onRefresh: () => void }) {
  ...
  return (
    <div className="space-y-4">
      <SectionHeader icon={ArchiveRestore} color="#f59e0b" label={t("claudeConfig.trash.title")} desc={t("claudeConfig.sectionDesc.trash")}
